...
function hasData() {
    const row = (0, db_1.get)('SELECT COUNT(*) as c FROM users');
    return (row?.c || 0) > 0;
}
const BOOKS = [
...
function seed() {
    ...
    (0, db_1.setSetting)('loanDays', 7);
    (0, db_1.setSetting)('finePerDay', 1000);
    (0, db_1.setSetting)('maxActiveLoans', 3);
    const now = (0, date_1.nowISO)();
    const adminHash = bcryptjs_1.default.hashSync('admin123', 10);
    const aRes = (0, db_1.run)('INSERT INTO users (nama, email, password_hash, role, no_anggota, phone, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', ['Petugas Perpustakaan', 'admin@pustaka.id', adminHash, 'admin', 'ADM-001', '081234567890', 'aktif', now]);
    (0, db_1.run)('UPDATE users SET no_anggota = ? WHERE id = ?', ['ADM' + String(aRes.lastId).padStart(3, '0'), aRes.lastId]);
    (0, db_1.pushNotification)(aRes.lastId, 'info', 'Akun admin berhasil dibuat. Default: admin@pustaka.id / admin123');
    const memberData = [
    ...
    for (const [nama, email, phone, noAnggota] of memberData) {
        const hash = bcryptjs_1.default.hashSync('member123', 10);
        const res = (0, db_1.run)('INSERT INTO users (nama, email, password_hash, role, no_anggota, phone, status, created_at) VALUES (?, ?, ?, \'member\', ?, ?, \'aktif\', ?)', [nama, email, hash, null, phone || null, now]);
        const no = noAnggota || ('A' + String(res.lastId).padStart(4, '0'));
        (0, db_1.run)('UPDATE users SET no_anggota = ? WHERE id = ?', [no, res.lastId]);
        memberIds.push(res.lastId);
        (0, db_1.pushNotification)(res.lastId, 'info', 'Selamat datang di Pustaka QR! Kartu anggota digital Anda tersedia di halaman profil.');
    }
    const bookIds = [];
    for (let i = 0; i < BOOKS.length; i++) {
        const b = BOOKS[i];
        const res = (0, db_1.run)(`INSERT INTO books (judul, penulis, penerbit, tahun, kategori, isbn, cover_url, lokasi_rak, deskripsi, qr_code, stok_total, stok_tersedia, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [b.judul, b.penulis, b.penerbit, b.tahun, b.kategori, b.isbn, cover(i), b.lokasi_rak, b.deskripsi, null, b.stok, b.stok, now]);
        (0, db_1.run)('UPDATE books SET qr_code = ? WHERE id = ?', [(0, qr_1.bookCode)(res.lastId), res.lastId]);
        bookIds.push(res.lastId);
    }
    const pinjam = (userId, bookIdx, daysAgo, dueIn) => {
        const bookId = bookIds[bookIdx];
        const tPinjam = (0, date_1.addDaysISO)((0, date_1.nowISO)(), -daysAgo);
        const tDue = (0, date_1.addDaysISO)((0, date_1.nowISO)(), dueIn);
        (0, db_1.run)('INSERT INTO loans (user_id, book_id, tanggal_pinjam, tanggal_jatuh_tempo, status, created_at) VALUES (?, ?, ?, ?, ?, ?)', [userId, bookId, tPinjam, tDue, 'dipinjam', tPinjam]);
    ...
    db_1.db.prepare(`UPDATE loans SET status = 'terlambat' WHERE tanggal_jatuh_tempo < datetime('now') AND status='dipinjam'`).run();
    const finir = (userId, bookIdx, pinjamDaysAgo, kembaliDaysAgo) => {
        const tPinjam = (0, date_1.addDaysISO)((0, date_1.nowISO)(), -pinjamDaysAgo);
        const tDue = (0, date_1.addDaysISO)(tPinjam, 7);
        const tKembali = (0, date_1.addDaysISO)((0, date_1.nowISO)(), -kembaliDaysAgo);
        const late = Math.max(0, kembaliDaysAgo - (pinjamDaysAgo - 7));
        const { lastId } = (0, db_1.run)('INSERT INTO loans (user_id, book_id, tanggal_pinjam, tanggal_jatuh_tempo, tanggal_kembali, status, created_at) VALUES (?, ?, ?, ?, ?, \'selesai\', ?)', [userId, bookIdx, tPinjam, tDue, tKembali, tPinjam]);
        if (late > 0) {
            const denda = late * 1000;
            (0, db_1.run)('INSERT INTO fines (loan_id, user_id, jumlah, hari_terlambat, status_bayar, created_at) VALUES (?, ?, ?, ?, ?, ?)', [lastId, userId, denda, late, late > 14 ? 'lunas' : 'belum', tKembali]);
        }
    };
    ...
    (0, db_1.run)('INSERT INTO reservations (user_id, book_id, tanggal_reservasi, status, created_at) VALUES (?, ?, ?, \'menunggu\', ?)', [memberIds[1], bookIds[6], now, now]);
    (0, db_1.run)('INSERT INTO reservations (user_id, book_id, tanggal_reservasi, status, created_at) VALUES (?, ?, ?, \'menunggu\', ?)', [memberIds[0], bookIds[2], now, now]);
    const reviews = [
    ...
    for (const [u, b, rating, ulasan] of reviews) {
        (0, db_1.run)('INSERT INTO reviews (user_id, book_id, rating, ulasan, created_at) VALUES (?, ?, ?, ?, ?)', [u, bookIds[b], rating, ulasan, now]);
    }
