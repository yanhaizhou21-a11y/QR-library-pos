...
exports.db = new node_sqlite_1.DatabaseSync(node_path_1.default.join(dataDir, 'pustaka.db'));
...
function all(sql, params = []) {
    return exports.db.prepare(sql).all(...params);
}
function get(sql, params = []) {
    return exports.db.prepare(sql).get(...params);
}
function run(sql, params = []) {
    const res = exports.db.prepare(sql).run(...params);
...
function settingsSnapshot() {
    ...
    const merged = { ...config_1.DEFAULTS };
    for (const r of rows)
        merged[r.key] = r.value;
    return merged;
}
function pushNotification(userId, tipe, pesan, refId) {
    run('INSERT INTO notifications (user_id, tipe, pesan, ref_id, is_read, created_at) VALUES (?, ?, ?, ?, 0, datetime(\'now\'))', [userId, tipe, pesan, refId ?? null]);
}
