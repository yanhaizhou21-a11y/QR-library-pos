...
function useCheckboxGroupParent(params) {
  ...
  const registerChildId = (0, _useStableCallback.useStableCallback)((childValue, childId) => {
  ...
  const getParentProps = React.useCallback(() => ({
    ...
    // element is named.
    'aria-controls': allValues.flatMap(v => childIdsState.registry.get(v) ?? _empty.EMPTY_ARRAY).join(' ') || undefined,
